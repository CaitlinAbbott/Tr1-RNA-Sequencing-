...
```{r heatmap-top100, fig.height=4, fig.cap="The 100 genes showing the most extreme fold-change across one or more comparisons"}
top_100 <- top_tables %>% 
...
p <- grp_coef[top_100,] %>% 
  ...
  pheatmap(
    ...
    labels_col = id2gene[top_100],
    labels_row = c(DN = "DN", DP = "DP", "LAG_3" = "Lag3+", CD49b = "Cd49b+"),
    cluster_rows = FALSE,
  ...
  .[["gtable"]]
p$grobs[[3]]$gp <- gpar(fontsize = 8, fontface = "italic")
p$grobs[[4]]$gp <- gpar(fontsize = 10)
grid.newpage()
grid.draw(p)
```
...
![Expression patterns across all groups for key marker genes](assets/highconf_heat.png)
