...
```{r upset-all-de, fig.height=7, fig.cap = "UpSet plot for all DE genes. A complete list of DE genes was obtained by finding all genes considered DE after filtering by FDR and logFC. For the purposes of comparison, any genes in this list which received an FDR-adjusted p-value < 0.05 in any comparison were included as DE for this figure to give a mroe accurate picture across all 6 comparisons."}
all_de <- top_tables %>% 
...
tbl <- top_tables %>% 
  bind_rows(.id = "comparison") %>% 
  # dplyr::filter(adj.P.Val < 0.05, gene_id %in% all_de) %>% 
  dplyr::filter(gene_id %in% all_de) %>%
  dplyr::select(comparison, gene_name, logFC) %>% 
  # group_by(gene_name) %>% 
  # mutate(n = dplyr::n()) %>% 
  # ungroup() %>% 
  ...
  reactable(
    filterable = TRUE,
    columns = list(
      gene_name = colDef(name = "Gene", maxWidth = 200),
      # n = colDef(name = "# Comps", format = colFormat(digits = 0)),
      DNvLAG3 = colDef(
        name = "DN Vs. LAG3",
    ...
    defaultColDef = colDef(format = colFormat(digits = 2), maxWidth = 100)
  )
cp <- htmltools::tags$caption(
  htmltools::em(
    glue(
      "
      Interactive summary table of all {length(all_de)} genes considered as DE 
      in at least one comparison. Estimated logFC values are given regardless 
      of formal consideration s DE.
      "
    )
  )
)
htmltools::div(
  id = "tab-all-de",
  htmltools::div(class = 'table-caption', cp),
